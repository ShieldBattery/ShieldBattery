...
ShieldBattery is a combination of Rust and TypeScript/JavaScript, split between multiple server
and client pieces. Even if you only plan on developing TypeScript changes, you'll need the native
build tooling below in order to properly test things.
...
The code that runs within the BW process (the `game/` directory) and the `server-rs` GraphQL
...
Along with nodejs, the server requires [PostgreSQL 17](https://postgresql.org) (the version
production runs), and [redis](https://redis.io).
...
If you don't already have `docker` (with the Compose plugin) installed, you'll need to follow the
...
Redis has no official Windows build (the old MSOpenTech installers are long abandoned), so on
Windows use the Docker setup above or run it under WSL. On Linux, it can generally be installed
through the package manager for your OS. On Mac, use [brew](http://brew.sh).

Start the redis server by using
...
```
node -r @swc-node/register ./server/app.ts
```
...
- **Build time**: `SB_SERVER` set in the environment that runs the Electron client build
  (`pnpm run build-app-client`) or the renderer dev server (`pnpm run dev`) will pick the
...
Additionally, any arguments other than `nobuild` will be forwarded to playwright, which allows you
