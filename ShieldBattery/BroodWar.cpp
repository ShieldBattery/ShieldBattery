...
void BroodWar::ApplyPatches() { // TODO(tec27): yeah....
  ...
  unsigned char* snp_patch = 
      reinterpret_cast<unsigned char*>(reinterpret_cast<unsigned int>(storm) + 
      reinterpret_cast<unsigned int>(offsets_->storm_unsigned_snp_patch));
...
void __stdcall DummyMapListEntryCallback(MapListEntry* map_data, char* map_name, int file_type) {
}
...
void BroodWar::GetMapsList(const MapListEntryCallback callback) {
  ...
  GetMapsListFunc get_list = offsets_->GetMapsList;
  int unk1 = 0x28; // 0x10 can be used for a recent maps list instead
...
void BroodWar::SelectMapOrDirectory(const std::string& game_name, const std::string& password,
  ...
  SelectMapOrDirectoryFunc select = offsets_->SelectMapOrDirectory;
