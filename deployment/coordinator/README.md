...
- `config/` — mounted read-only into the coordinator. `tenants.json` (tenant
  ...
  names the `.env` variable holding its signing key) is the hand-edited one.
  `regions.json` (region registry) and `ecs.json` (per-region Fargate launch
  config) are both generated by the relay-fleet Terraform — regions are
  declared once in its region catalog, never here by hand. Samples included
  for shape reference.
