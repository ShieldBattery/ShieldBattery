...
export default class PsiHealthCheckupDialog extends React.Component {
  ...
  renderStarcraftVersionInfo() {
    ...
    if (downgradeInProgress) {
      return (<div>
        <p>ShieldBattery is attempting to identify the version of your installed StarCraft
          client&hellip;</p>
    ...
    if (!lastDowngradeError ||
        (lastDowngradeError.body &&
            lastDowngradeError.body.error === 'Unrecognized file version')) {
      return (<div>
        <p>
    ...
    return (<div>
      <p>ShieldBattery was unable to make your version of StarCraft compatible with the service. The
        error message was:</p>
      <p className={styles.errorMessage}>
        {lastDowngradeError.body ? lastDowngradeError.body.error : lastDowngradeError.message}
      </p>
      <p>This may be a temporary error. Click the button below to try again, or consider
        reinstalling the <a href={STARCRAFT_DOWNLOAD_URL} target='_blank'>latest version</a> of
        StarCraft and restarting ShieldBattery.</p>
      <RaisedButton label={'Try again'} onClick={this.onTryAgainClick} />
    </div>)
  }
