 ...
 * shipped as an Electron asset (`app/assets/minimaps`) rather than a server-hosted public asset --
 * this preview only ever renders inside the Electron app (see the `IS_ELECTRON` gate around the
 * Game settings section in `client/settings/settings.tsx`), so it's loaded straight off disk
 * through the `shieldbattery://` protocol (see `app/app.ts`'s asset protocol handler).
 */
function getMinimapImageUrl(tileset: TilesetId): string {
  return `shieldbattery://app/assets/minimaps/${tileset}.webp`
}
