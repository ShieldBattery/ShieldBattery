...
export default class UpdateDialog extends React.Component {
  ...
  renderInfo() {
    if (this.props.update.hasDownloadError) {
      return (<p className={styles.text}>There was an error downloading the latest update. Please
          restart and try again.</p>)
    } else if (this.props.update.readyToInstall) {
      return [
        <p className={styles.text} key={'text'}>A new update is downloaded and ready to install.
            Please restart the application to continue.</p>,
        <RaisedButton key={'button'} onClick={this.onRestartClick} label={'Restart now'} />,
      ]
    } else {
      return [
        <p className={styles.text} key={'text'}>A new update is available and is downloading. Please
            wait for the download to complete in order to continue.</p>,
        <div key={'loading'} className={styles.loading}><LoadingIndicator/></div>,
      ]
    }
  }
  ...
  render() {
    ...
    return (<Dialog title={title} modal={true} showCloseButton={false}>
      { this.renderInfo() }
    </Dialog>)
  }
