...
/**
 * Loads (and caches) the shortcode dataset, keyed by lowercased unified hexcode to its primary
 * shortcode. Safe to call repeatedly; concurrent callers share the same in-flight load.
 */
  ...
  pendingLoad ??= import('emojibase-data/en/shortcodes/iamcal.json').then(
    module => {
      ...
      for (const [hexcode, value] of Object.entries(data)) {
        ...
        map.set(hexcode.toLowerCase(), primary)
      }
      cache = map
      return map
    },
...
/**
 * Looks up the shortcode for a dash-separated unified hexcode (as found in the emoji picker
 * dataset's `u` field, e.g. "1f605" or "263a-fe0f"). Returns `undefined` both when there's no
 * match and when the dataset hasn't finished loading yet (call `loadShortcodes` first).
 */
