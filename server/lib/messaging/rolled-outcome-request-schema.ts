 ...
 *
 * `max` and `question` each belong to exactly one kind and are refused on the others, so a question
 * can't ride along on a roll and be stored as the message's text without the 8-ball having been
 * asked anything.
 */
export const ROLLED_OUTCOME_REQUEST_KEYS: Joi.SchemaMap<RolledOutcomeRequestFields> = {
  kind: Joi.string().valid('roll', 'flip', 'eightBall').required(),
